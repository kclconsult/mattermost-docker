...
docker-compose exec app mattermost config set ServiceSettings.AllowedUntrustedInternalConnections "localhost"
...
docker-compose exec app mattermost command create patient --title start --description "start" --trigger-word start --url https://device-integration_nokia_proxy_1/dialogue/response --creator connie --response-username connie --autocomplete --post
